...
import { collection, addDoc, serverTimestamp, getDocs, query, writeBatch, doc, updateDoc, onSnapshot, where } from "firebase/firestore";
...
const AddItem = ({ editData, onComplete }) => {
  ...
  const handleExcelImport = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        // Fetch all existing names to check for duplicates
        const existingSnapshot = await getDocs(collection(db, "inventory_records"));
        const existingNames = existingSnapshot.docs.map(d => d.data().name.toUpperCase());
        ...
        data.forEach((item) => {
          ...
          const purP = item["Purchase Price"] || 0;
          const minS = item["Min Stock"] || 0;
          const maxS = item["Max Stock"] || 0;
          const skuGenerated = `${name.substring(0, 3)}-${sr}`;

          batch.set(newDocRef, {
            name, srNo: sr, sku: skuGenerated,
            ...
            minStock: minS, maxStock: maxS,
            barcodeData: `SN:${sr}|SKU:${skuGenerated}|PCS:${pBox}|VOL:${L}x${W}x${H}|WT:${wt}KG|PUR:${purP}|MIN:${minS}|MAX:${maxS}`,
            qrCodeData: `ITEM:${name}|WH:${(item["Warehouse"] || "MAIN STORE").toUpperCase()}|TPCS:${openStock * pBox}`,
            createdAt: serverTimestamp()
          });
        });
        await batch.commit();
        setStatusMessage(skipCount > 0 ? `DONE (${skipCount} DUPS SKIPPED)` : "SUCCESS!");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!editData) {
        const qCheck = query(collection(db, "inventory_records"), where("name", "==", nameUpper));
        const checkSnap = await getDocs(qCheck);
        if (!checkSnap.empty) {
          alert("Error: Product Name already exists! Duplicates not allowed.");
          setStatusMessage('');
          return;
        }
      }
    ...
    <div className="add-item-container">
      ...
      <div className="layout-grid">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="input-group-row">
                <div className="flex-1-5"><label className="label-text">Company *</label>
                  <div style={{display:'flex'}}><input list="company-list" className="custom-input" value={formData.company} onChange={e=>setFormData({...formData, company: e.target.value.toUpperCase()})} placeholder="Search/Select" required /><datalist id="company-list">{companies.map(c=><option key={c} value={c} />)}</datalist>
                  <button type="button" onClick={()=>setCompanies([...companies, prompt("New Company")?.toUpperCase()])} className="btn-plus">+</button></div>
                </div>
                <div className="flex-1-5"><label className="label-text">Category *</label>
                  <div style={{display:'flex'}}><input list="category-list" className="custom-input" value={formData.category} onChange={e=>setFormData({...formData, category: e.target.value.toUpperCase()})} placeholder="Search/Select" required /><datalist id="category-list">{categories.map(c=><option key={c} value={c} />)}</datalist>
                  <button type="button" onClick={()=>setCategories([...categories, prompt("New Category")?.toUpperCase()])} className="btn-plus">+</button></div>
                </div>
              </div>
              <div className="input-group-row">
                <div className="flex-1-5"><label className="label-text">Sub-Category *</label>
                  <div style={{display:'flex'}}><input list="sub-list" className="custom-input" value={formData.subCategory} onChange={e=>setFormData({...formData, subCategory: e.target.value.toUpperCase()})} placeholder="Search/Select" required /><datalist id="sub-list">{subCategories.map(c=><option key={c} value={c} />)}</datalist>
                  <button type="button" onClick={()=>setSubCategories([...subCategories, prompt("New Sub-Category")?.toUpperCase()])} className="btn-plus">+</button></div>
                </div>
                <div className="flex-1-5"><label className="label-text">Warehouse *</label>
                  <div style={{display:'flex'}}><input list="wh-list" className="custom-input" value={formData.warehouse} onChange={e=>setFormData({...formData, warehouse: e.target.value.toUpperCase()})} placeholder="Search/Select" required /><datalist id="wh-list">{warehouses.map(w=><option key={w} value={w} />)}</datalist>
                  <button type="button" onClick={()=>setWarehouses([...warehouses, prompt("New Warehouse")?.toUpperCase()])} className="btn-plus">+</button></div>
                </div>
              </div>
              <div className="input-group-row">
        ...
        <div className="preview-card">
          ...
          <div className="preview-data-box"><label className="label-text" style={{color: '#f59e0b'}}>Barcode Data</label><div style={{fontSize:'10px', color:'#888', wordBreak: 'break-all'}}>{formData.barcodeData}</div></div>
          <div className="preview-data-box"><label className="label-text" style={{color: '#f59e0b'}}>QR Data</label><div style={{fontSize:'10px', color:'#888', wordBreak: 'break-all'}}>{formData.qrCodeData}</div></div>
