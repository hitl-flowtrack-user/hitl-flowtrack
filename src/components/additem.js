...
const AddItem = ({ editData, onComplete }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Duplicate Name Check (Only for New Items)
      if (!editData) {
        const qName = query(collection(db, "inventory_records"), where("name", "==", formData.name.toUpperCase()));
        const nameCheck = await getDocs(qName);
        if (!nameCheck.empty) {
          alert("ERROR: This product name already exists! Please use a unique name.");
          setStatusMessage('');
          return;
        }
      }
        ...
        <div className="form-card">
          <div className="form-inner-container">
            <form onSubmit={handleSubmit}>
              <div style={{marginBottom: '15px'}}><label className="label-text">Item Name *</label><input className="custom-input" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value.toUpperCase()})} required /></div>
              
              <div className="input-group-row">
                <div className="flex-1-5"><label className="label-text">Company *</label>
                  <div style={{display:'flex'}}><input list="co-list" className="custom-input" value={formData.company} onChange={e=>setFormData({...formData, company: e.target.value.toUpperCase()})} required /><datalist id="co-list">{companies.map(c=><option key={c} value={c}/>)}</datalist></div>
                </div>
                <div className="flex-1-5"><label className="label-text">Category *</label>
                  <div style={{display:'flex'}}><input list="cat-list" className="custom-input" value={formData.category} onChange={e=>setFormData({...formData, category: e.target.value.toUpperCase()})} required /><datalist id="cat-list">{categories.map(c=><option key={c} value={c}/>)}</datalist></div>
                </div>
              </div>

              <div className="input-group-row">
                <div className="flex-1-5"><label className="label-text">Sub-Category *</label>
                  <div style={{display:'flex'}}><input list="sub-list" className="custom-input" value={formData.subCategory} onChange={e=>setFormData({...formData, subCategory: e.target.value.toUpperCase()})} required /><datalist id="sub-list">{subCategories.map(c=><option key={c} value={c}/>)}</datalist></div>
                </div>
                <div className="flex-1-5"><label className="label-text">Warehouse *</label>
                  <div style={{display:'flex'}}><input list="wh-list" className="custom-input" value={formData.warehouse} onChange={e=>setFormData({...formData, warehouse: e.target.value.toUpperCase()})} required /><datalist id="wh-list">{warehouses.map(w=><option key={w} value={w}/>)}</datalist></div>
                </div>
              </div>

              <div className="input-group-row">
                <div className="flex-1"><label className="label-text">Stock (Boxes)</label><input type="number" className="custom-input" value={formData.openingStock} onChange={e=>setFormData({...formData, openingStock: e.target.value})} /></div>
              ...
              <div style={{marginBottom: '15px'}}><label className="label-text">Purchase Price *</label><input type="number" className="custom-input" value={formData.purchasePrice} onChange={e=>setFormData({...formData, purchasePrice: e.target.value})} required /></div>
