...
const AddItem = ({ editData, onComplete }) => {
  ...
  const styles = `
    ...
    /* 3x Plus Button */
    .btn-plus { background-color: #f59e0b; color: #000; width: 60px; height: 50px; border: none; border-radius: 10px; font-weight: bold; cursor: pointer; margin-left: 8px; font-size: 24px; display: flex; align-items: center; justify-content: center; }
    ...
    <div className="add-item-container">
      ...
      <div className="layout-grid">
        <div className="form-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '30px', width: '60%' }}>
            <h2 style={{ fontStyle: 'italic', fontWeight: '900', color: '#f59e0b', margin: 0 }}>FLOWTRACK</h2>
            <div>
              <button type="button" className="btn-top" onClick={() => excelInputRef.current.click()}>Import</button>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="grid-row-all">
            {/* SR and SKU Opposite in One Line */}
        ...
        <div className="preview-card">
          ...
          <div className="preview-data-box">
            <label className="label-text" style={{fontSize: '9px', color: '#f59e0b'}}>Barcode Data</label>
            <div style={{fontSize:'11px', color:'#fff', wordBreak: 'break-all'}}>{formData.barcodeData}</div>
          </div>
          
          <div className="preview-data-box">
            <label className="label-text" style={{fontSize: '9px', color: '#f59e0b'}}>QR Data</label>
