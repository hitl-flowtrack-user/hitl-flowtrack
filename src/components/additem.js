...
import { db } from '../firebase'; // Path check karein aapke folder ke mutabiq
  ...
  return (
    <div className="flex flex-col gap-1 relative" ref={wrapperRef}>
      <div className="flex items-center justify-between px-1">
        ...
        {isSuperAdmin && <span className="text-[10px] text-amber-600/50 uppercase tracking-tighter">Super Admin Mode</span>}
      </div>
...
const InputField = ({ label, value, onChange, readOnly, type="text", required, errorKey, hasError, clearError, isUppercase }) => (
  <div className="flex flex-col gap-1">
    ...
    <input 
      ...
      className={`bg-[#FFFFFF] rounded-2xl p-3 text-sm text-black outline-none shadow-inner transition-all border-2 ${isUppercase ? 'uppercase' : ''} ${readOnly ? 'opacity-70 italic border-transparent bg-zinc-100' : (hasError ? 'border-red-500 ring-2 ring-red-500/20' : 'border-transparent focus:border-amber-500')}`}
    />
  </div>
);
...
const AddItem = ({ editData, onComplete }) => {
  ...
  useEffect(() => {
    if (formData.name.length >= 1 && !editData) {
      ...
      const qrFilter = `NM:${formData.name.toUpperCase()} | CO:${formData.company || 'NA'} | PRC:${formData.retailPrice || '0'}`;
      setFormData(prev => ({ 
          ...prev, 
          sku: generatedSKU, 
  ...
  const downloadExcelTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Elite_Template");
      ...
      try {
        ...
        data.forEach(item => {
          if (item.Name) {
            ...
            const ref = doc(collection(db, "inventory_records")); // Consistent collection name
            batch.set(ref, { 
                ...item, 
                srNo: sr, 
  ...
  return (
    <div className="min-h-screen bg-[#000000] flex items-center justify-center p-4 lg:p-8 font-sans">
        ...
        <div className="bg-[#121212] rounded-[2.5rem] p-8 lg:p-10 shadow-2xl flex flex-col gap-6 w-full border border-white/5">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex gap-2">
                ...
                <button type="button" onClick={() => excelInputRef.current.click()} className="bg-[#2a2a2a] text-zinc-300 px-4 py-1.5 rounded-full text-[10px] font-bold border border-white/5 uppercase">Excel Import</button>
            </div>
          </div>
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            {/* New Barcode and QR Text Fields (Read Only) */}
            <div className="grid grid-cols-2 gap-4">
                <InputField label="Barcode Value" value={formData.barcodeData} readOnly />
                <InputField label="QR Value" value={formData.qrCodeData} readOnly />
            </div>
        ...
        <div className="flex items-center justify-center lg:sticky lg:top-8 w-full">
          <div className="bg-[#121212] w-full max-w-[420px] rounded-[3.5rem] p-8 shadow-2xl relative border border-white/5">
            ...
            <div className="space-y-1 mb-8 text-center lg:text-left">
              ...
              <p className="text-amber-500 font-black text-[11px] uppercase tracking-[0.2em]">{formData.company || "Company ID"}</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
      ...
      <style dangerouslySetInnerHTML={{ __html: `@import url('https://fonts.googleapis.com/css2?family=Noto+Nastaliq+Urdu&display=swap');
